<script setup lang="ts">
import { ref } from "vue";
import { getCourses } from "./api";
import CourseCard from "./components/CourseCard.vue";
import RadioButtons from "./components/RadioButtons.vue";
import AppDrawer from "./components/AppDrawer.vue";

const today = new Date();
const offset = today.getTime() - today.getTimezoneOffset() * 60 * 1000;
const minDate = new Date(offset);
const minDateStr = minDate.toISOString().substring(0, 10);
const maxDate = new Date(offset);
maxDate.setDate(minDate.getDate() + 14);
const maxDateStr = maxDate.toISOString().substring(0, 10);
const date = ref(minDateStr);

const courses = ref<Course[]>([]);

const loadCourses = async () => {
    const coursesResponse = await getCourses();
    courses.value = coursesResponse
        .sort((a, b) => a.name.localeCompare(b.name))
        .map((c) => ({
            ...c,
            teeTimes: [],
            loading: false,
        }));
};

loadCourses();

const holesFilter = ref("any");
const playersFilter = ref("any");
const filtersOpen = ref(false);
</script>

<template>
    <div
        class="min-h-full flex flex-col bg-brand-700 text-gray-50"
        :class="{ 'drawer-open': filtersOpen }"
    >
        <nav
            class="bg-brand-700 border-b fixed top-0 left-0 right-0 flex items-center justify-between z-40 h-12 px-3 p-1"
        >
            <h1 class="text-white font-semibold">Golf KC</h1>
            <a href="https://github.com/sdbrannum" target="_blank">
                <svg
                    class="w-6 h-6"
                    xmlns="http://www.w3.org/2000/svg"
                    viewBox="0 0 100 100"
                >
                    <path
                        fill-rule="evenodd"
                        clip-rule="evenodd"
                        d="M48.854 0C21.839 0 0 22 0 49.217c0 21.756 13.993 40.172 33.405 46.69 2.427.49 3.316-1.059 3.316-2.362 0-1.141-.08-5.052-.08-9.127-13.59 2.934-16.42-5.867-16.42-5.867-2.184-5.704-5.42-7.17-5.42-7.17-4.448-3.015.324-3.015.324-3.015 4.934.326 7.523 5.052 7.523 5.052 4.367 7.496 11.404 5.378 14.235 4.074.404-3.178 1.699-5.378 3.074-6.6-10.839-1.141-22.243-5.378-22.243-24.283 0-5.378 1.94-9.778 5.014-13.2-.485-1.222-2.184-6.275.486-13.038 0 0 4.125-1.304 13.426 5.052a46.97 46.97 0 0 1 12.214-1.63c4.125 0 8.33.571 12.213 1.63 9.302-6.356 13.427-5.052 13.427-5.052 2.67 6.763.97 11.816.485 13.038 3.155 3.422 5.015 7.822 5.015 13.2 0 18.905-11.404 23.06-22.324 24.283 1.78 1.548 3.316 4.481 3.316 9.126 0 6.6-.08 11.897-.08 13.526 0 1.304.89 2.853 3.316 2.364 19.412-6.52 33.405-24.935 33.405-46.691C97.707 22 75.788 0 48.854 0z"
                        fill="#fff"
                    />
                </svg>
            </a>
        </nav>
        <main class="flex-1 flex flex-col items-center p-2 pt-14">
            <course-card
                v-for="course in courses"
                :key="course.id"
                v-bind="course"
                :players="playersFilter"
                :holes="holesFilter"
                :date="date"
            />
        </main>
        <app-drawer v-model="filtersOpen" id="drawer" class="text-gray-800">
            <div class="pb-2">
                <input
                    class="block rounded mr-2 md:w-64"
                    type="date"
                    :min="minDateStr"
                    :max="maxDateStr"
                    v-model="date"
                />
            </div>
            <div class="pb-2">
                <h5
                    class="text-base font-semibold text-gray-500 dark:text-gray-400"
                >
                    Players
                </h5>
                <radio-buttons
                    :name="'players'"
                    :items="['any', '1', '2', '3', '4']"
                    v-model="playersFilter"
                />
            </div>
            <div class="pb-2">
                <h5
                    class="text-base font-semibold text-gray-500 dark:text-gray-400"
                >
                    Holes
                </h5>
                <radio-buttons
                    :name="'holes'"
                    :items="['any', '9', '18']"
                    v-model="holesFilter"
                />
            </div>
        </app-drawer>
    </div>
</template>

<style scoped>
.list-enter-active,
.list-leave-active {
    transition: all 0.5s ease;
}

.list-enter-from {
    opacity: 0;
    transform: translateX(30px);
}

.list-leave-to {
    opacity: 0;
    transform: translateX(-30px);
}

.list-leave,
.list-leave-active {
    position: absolute;
}

main {
    padding-bottom: 64px;
}
.drawer-open main {
    padding-bottom: 218px;
}
</style>
